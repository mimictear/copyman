...
public struct TagLayout: Layout {
    ...
    var spacing: CGFloat?
    var stickLastViewToRightEdge: Bool
    
    public init(alignment: Alignment, spacing: CGFloat? = nil, stickLastViewToRightEdge: Bool = false) {
        self.alignment = alignment
        self.spacing = spacing
        self.stickLastViewToRightEdge = stickLastViewToRightEdge
    }
    ...
    public func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout Void) {
        ...
        for row in result.rows {
            ...
            for index in row.range {
                ...
                if index == (subviews.endIndex - 1), stickLastViewToRightEdge {
                    let hPadding = UIScreen.main.bounds.minX + bounds.minX
                    let lastViewWidth = subviews[index].dimensions(in: .unspecified).width
                    let xGap = (UIScreen.main.bounds.size.width - lastViewWidth) - hPadding
                    subviews[index].place(at: CGPoint(x: xGap, y: yPos), anchor: .topLeading, proposal: .unspecified)
                } else {
